...
namespace sfe
{
    ...
    std::vector<sf::Text>& RichText::Line::getTexts() const
    {
        return m_texts;
    }

    void RichText::Line::insertString(std::size_t pos, sf::String string) const
    {
        if (pos >= getLength()) return;

        sf::Text& text = m_texts[convertLinePosToLocal(pos)];
        alce::String str = alce::String(text.getString().toAnsiString());
        
        str.Insert(pos, alce::String(string.toAnsiString()));
    ...
    std::string RichText::getString() const
    {
        std::string str = "";

        for (std::size_t i = 0; i < getLines().size(); i++)
        {
            str += getStringOfLine(i);
        }
    ...
    std::string RichText::getStringOfLine(std::size_t line) const
    {
        if (line >= getLines().size()) return "";

        std::string lineString = "";

        for (auto& i : getLines()[line].getTexts())
        {
